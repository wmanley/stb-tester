...
def svg_to_array(svg):
    ...
    return stbt.gst_to_opencv(sample)
...
def generate_video(filename, colours, *args, **kwargs):
    import gst_utils
    gst_utils.generate_svg_video(
        filename, [(generate_letters_svg(*colours), 120 * Gst.SECOND * 2)],
        *args, **kwargs)
...
def validate(video, driver):
    ...
    for square, char in zip(SQUARES, GLYPHS):
        ...
        result = (stbt.detect_match(
            template,
            match_parameters=MatchParameters(match_method="ccoeff-normed"))
            .next())
        r = distance(pos, result.position)
        sys.stdout.write(
    ...
    for square, char, result in res:
        ...
        if length(off) > 0.5:
            ...
            arrow = arrows[int(angle/2/math.pi*len(arrows)+len(arrows)+0.5)
                           % len(arrows)]
        else:
            arrow = 'O'
        sys.stdout.write(
...
def main(argv):
    ...
    if args.generate_reference:
        ...
        video_generators = {}
        for desc, colour in STANDARD_COLOURS.items():
            video_generators[desc] = \
                lambda filename, c=colour, *args, **kwargs: \
                    generate_video(filename, c, *args, **kwargs)
        driver = tv_driver.create_from_args(args, video_generators)
        failures = 0
