...
_libstbt = ctypes.CDLL(_find_file("libstbt-30.so"))
...
def test_sqdiff():
    ...
    assert (0, 1280 * 720) == _sqdiff_c(t, f)
    assert (0, 1280 * 720) == _sqdiff_c(tt, f)
    assert (0, 1280 * 720) == _sqdiff_c(tt[:,:,:3], f)

    f = numpy.ones((720, 1280, 3), dtype=numpy.uint8) * 255
    t = numpy.zeros((720, 1280, 3), dtype=numpy.uint8)
    tt = numpy.zeros((720, 1280, 4), dtype=numpy.uint8)

    assert (1280 * 720 * 255 * 255 * 3, 1280 * 720) == _sqdiff_c(t, f)
    assert (1280 * 720 * 255 * 255 * 3, 1280 * 720) == _sqdiff_c(t, f)
    ...
    assert (1280 * 720 * 255 * 255 * 3, 1280 * 720) == _sqdiff_c(t, f)
    assert (1280 * 720 * 255 * 255 * 3, 1280 * 720) == _sqdiff_c(t, f)
    assert (1280 * 720 * 255 * 255 * 3, 1280 * 720) == _sqdiff_c(tt, f)
    ...
    for x in range(100):
        ...
        for l, t in [("template ", template),
            ...
            np_time = min(timeit.repeat(
                lambda: _sqdiff_numpy(template, frame_cropped),
                repeat=3, number=10)) / 10
            c_time = min(timeit.repeat(
                lambda: _sqdiff_c(template, frame_cropped),
                repeat=3, number=10)) / 10
            if _sqdiff_numba:
                numba_time = min(timeit.repeat(
                    lambda: _sqdiff_numba(template, frame_cropped),
                    repeat=3, number=10)) / 10
            else:
